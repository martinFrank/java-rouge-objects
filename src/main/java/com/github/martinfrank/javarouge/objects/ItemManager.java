...
public class ItemManager {
    ...
    private final ItemsampleDao itemDao;
    ...
    public ItemManager(ConfigurationProvider configurationProvider, ObjectConstants objectConstants) {
        ...
        itemDao = new ItemsampleDao(configurationProvider.getConfiguration());
    ...
    public ItemPrototype getItem(String name) {
        //FIXME Lookup Table
        Itemsample item = itemDao.fetchOne(Tables.ITEMSAMPLE.NAME, name);
    ...
    private void addSuitableSlots(ItemPrototype itemPrototype) {
        ...
        suitableslotsDao.fetchByItem(itemPrototype.getItemsample().getId()).forEach(s -> equipmentSlots.add(objectConstants.getEquipmentTypeSlotById(s.getEquipmentslot())));
    ...
    private void addApplicableSkills(ItemPrototype itemPrototype) {
        ...
        applicableskillsDao.fetchByItem(itemPrototype.getItemsample().getId()).forEach(s -> applicableSkills.add(objectConstants.getSkillById(s.getSkill())));
    ...
    private void addSkillModifier(ItemPrototype itemPrototype) {
        ...
        skillModifierDao.fetchByItem(itemPrototype.getItemsample().getId()).forEach(
    ...
    private void addStatModifier(ItemPrototype itemPrototype) {
        ...
        statModifierDao.fetchByItem(itemPrototype.getItemsample().getId()).forEach(
    ...
    public Itemsample getItemById(Long id) {
        //FIXME Lookup Table
        return itemDao.fetchOne(Tables.ITEMSAMPLE.ID, id);
    }
}
